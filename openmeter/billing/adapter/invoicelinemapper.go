...
func (a *adapter) mapInvoiceLineFromDB(in mapInvoiceLineFromDBInput) ([]*billing.Line, error) {
	// let's filter for deleted lines
	dbLines := in.lines

	if !in.includeDeleted {
		dbLines = lo.Filter(dbLines, func(l *db.BillingInvoiceLine, _ int) bool {
	...
	for _, dbLine := range dbLines {
		...
		for _, dbDetailedLine := range dbLine.Edges.DetailedLines {
			if !in.includeDeleted && dbDetailedLine.DeletedAt != nil {
				continue
			}
